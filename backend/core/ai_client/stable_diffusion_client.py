        ...
        **kwargs,
        ...
        try:
            ...
            if "images" not in result:
                return AIResponse(success=False, error=f"A111响应格式错误: {result}")

            # 提取图片数据（base64）
            images = result.get("images", [])
            if not images:
                return AIResponse(success=False, error="未生成图片")
            ...
            return AIResponse(
                success=True,
                data={"url": image_url, "images": [image_url]},
                metadata={
                    ...
                    "seed": result.get("info", {}).get("all_seeds", [seed])[0]
                    if seed == -1
                    else seed,
    ...
    def _generate_stability_ai(
        ...
        **kwargs,
        ...
        try:
            ...
            return AIResponse(
                success=True,
                data={"urls": image_urls, "images": artifacts},
                metadata={
                    "latency_ms": latency_ms,
                    "model": self.model_name,
                    "width": width,
    ...
    def validate_config(self) -> bool:
        ...
        try:
            if "sdapi" in self.api_url:
                ...
                api_url = self.api_url if self.api_url.endswith("/") else self.api_url + "/"
                response = requests.get(api_url + "sdapi/v1/sd-models", headers=headers, timeout=10)
                # 200或401都表示API可达
                ...
                response = requests.get(
                    "https://api.stability.ai/v1/user/balance", headers=headers, timeout=10
                )
                return response.status_code in [200, 401]
