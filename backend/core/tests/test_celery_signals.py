...
class TestTaskPrerunHandler:
    ...
    def test_logs_task_start(self, mock_logger):
        ...
        # 调用处理器
        task_prerun_handler(sender=mock_task, task_id="test-task-id", task=mock_task)
    ...
    def test_includes_task_arguments(self, mock_logger):
        ...
        task_prerun_handler(sender=mock_task, task_id="test-id", task=mock_task)
...
class TestTaskRetryHandler:
    ...
    def test_logs_task_retry(self, mock_logger):
        ...
        task_retry_handler(
            sender=mock_task, task_id="retry-id", reason="Connection timeout", einfo=None
        )
    ...
    def test_handles_missing_sender(self, mock_logger):
        """测试处理缺失sender的情况"""
        task_retry_handler(sender=None, task_id="retry-id", reason="Unknown error", einfo=None)
...
class TestTaskFailureHandler:
    ...
    def test_logs_task_failure(self, mock_logger):
        ...
        task_failure_handler(sender=mock_task, task_id="fail-id", exception=exception, einfo=None)
...
class TestFilterSensitiveKwargs:
    ...
    def test_filters_api_key(self):
        """测试过滤api_key字段"""
        kwargs = {"api_key": "sk-123456", "other": "value"}
    ...
    def test_filters_nested_sensitive_data(self):
        """测试过滤嵌套字典中的敏感数据"""
        kwargs = {"user": {"username": "admin", "password": "secret123"}}
    ...
    def test_case_insensitive_matching(self):
        """测试大小写不敏感匹配"""
        kwargs = {"PASSWORD": "secret", "Api_Key": "key123", "SECRET": "topsecret"}
    ...
    def test_preserves_non_sensitive_data(self):
        """测试保留非敏感数据"""
        kwargs = {"username": "admin", "email": "admin@example.com", "project_id": "123"}
