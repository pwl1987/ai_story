...
class TestErrorHandling:
    ...
    def test_handle_collection_error(self):
        """测试数据收集错误处理"""
        with patch.object(
            PercentileStats, "_collect_api_metrics", side_effect=Exception("Collection error")
        ):
            stats = PercentileStats.calculate_api_percentiles(15)
    ...
    def test_handle_calculation_error(self):
        """测试计算错误处理"""
        with patch.object(
            PercentileStats, "_calculate_percentiles", side_effect=Exception("Calculation error")
        ):
            stats = PercentileStats.calculate_api_percentiles(15)
