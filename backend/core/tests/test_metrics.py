...
class TestPrometheusMetrics:
    ...
    def test_celery_metrics_exist(self):
        """测试Celery指标存在"""
        assert celery_task_counter is not None
        assert celery_task_duration_histogram is not None
        assert celery_slow_task_counter is not None
...
class TestAPIRequestTracking:
    ...
    def test_track_slow_request(self):
        ...
        # 验证慢请求计数器增加
        samples = api_slow_request_counter.labels(
            method='GET',
...
class TestCeleryTaskTracking:
    ...
    def test_track_successful_task(self):
        ...
        # 验证任务被计数
        samples = celery_task_counter.labels(
            task_name='test_task',
            status='SUCCESS'
        )._samples()
        assert len(samples) > 0
    ...
    def test_track_failed_task(self):
        """测试跟踪失败的任务"""
        @track_celery_task('failing_task')
        def failing_task():
        ...
        # 验证失败任务被计数
        success_samples = celery_task_counter.labels(
        ...
        # 验证失败计数器
        failure_samples = celery_task_counter.labels(
            task_name='failing_task',
            status='FAILURE'
        )._samples()
    ...
    @pytest.mark.skipif(not PROMETHEUS_AVAILABLE, reason="prometheus_client not installed")
    def test_track_task_duration(self):
        """测试跟踪任务执行时间"""
        @track_celery_task('duration_task')
        def duration_task():
            import time
            time.sleep(0.1)
            return {'done': True}
        ...
        # 验证执行时间被记录
        samples = celery_task_duration_histogram.labels(
            task_name='duration_task'
        )._samples()
        assert len(samples) > 0
...
class TestMetricsWithoutPrometheusClient:
    ...
    @pytest.mark.skipif(PROMETHEUS_AVAILABLE, reason="prometheus_client is installed")
    def test_celery_decorator_works_without_prometheus(self):
        """测试Celery装饰器在没有prometheus_client时也能工作"""
        @track_celery_task('test_task')
        def task():
            return {'done': True}
