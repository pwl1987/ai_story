...
class TestMockLLMClient:
    ...
    def mock_llm_client(self):
        """创建MockLLMClient实例"""
        return MockLLMClient(api_url="mock://llm", api_key="mock_key", model_name="mock-llm")

    @pytest.mark.asyncio
    async def test_generate_rewrite_response(self, mock_llm_client):
        """测试生成改写响应"""
        response = await mock_llm_client.generate(
            prompt="请改写这个故事", max_tokens=2000, temperature=0.7
        )
    ...
    async def test_generate_storyboard_response(self, mock_llm_client):
        """测试生成分镜响应"""
        response = await mock_llm_client.generate(prompt="生成分镜", max_tokens=2000)
    ...
    async def test_generate_camera_movement_response(self, mock_llm_client):
        """测试生成运镜响应"""
        response = await mock_llm_client.generate(prompt="设计运镜", max_tokens=1000)
    ...
    async def test_generate_default_response(self, mock_llm_client):
        """测试生成默认响应"""
        response = await mock_llm_client.generate(prompt="随机内容")
    ...
    def test_generate_stream(self, mock_llm_client):
        """测试流式生成"""
        chunks = list(mock_llm_client.generate_stream(prompt="请改写这个故事"))
...
class TestMockText2ImageClient:
    ...
    def mock_image_client(self):
        ...
        return MockText2ImageClient(
            api_url="mock://text2image", api_key="mock_key", model_name="mock-text2image"
        )
...
class TestMockImage2VideoClient:
    ...
    def mock_video_client(self):
        ...
        return MockImage2VideoClient(
            api_url="mock://image2video", api_key="mock_key", model_name="mock-image2video"
        )
    ...
    async def test_generate_video(self, mock_video_client):
        ...
        response = await mock_video_client.generate(
            ...
            camera_movement={"movement_type": "slow_zoom_in", "duration": 3.0},
            duration=5.0,
            fps=24,
        )
