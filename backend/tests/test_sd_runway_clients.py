...
class TestStableDiffusionClient:
    ...
    def test_generate_a111_success(self, mock_post, client):
        ...
        mock_response.json.return_value = {
            "images": [
                "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAAAElFTkSuQmCC"
            ],
            "info": {"all_seeds": [12345]},
        }
        mock_post.return_value = mock_response

        result = client._generate_a111(prompt="测试提示词", width=512, height=512)
...
class TestRunwayClient:
    ...
    def client(self):
        return RunwayClient(
            api_url="https://api.runwayml.com/v1/", api_key="test-key", model_name="gen3a_turbo"
        )
    ...
    def test_submit_task_success(self, mock_post, client):
        ...
        mock_response.json.return_value = {"id": "test-task-123"}
        mock_post.return_value = mock_response

        task_id = client._submit_task(image_url="https://example.com/image.jpg", prompt="测试视频")
    ...
    def test_poll_task_success(self, mock_sleep, mock_get, client):
        ...
        mock_response_1.json.return_value = {"status": "PROCESSING"}
