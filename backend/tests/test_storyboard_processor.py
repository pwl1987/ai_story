...
class TestStoryboardProcessorGetInputData:
    ...
    def project_with_rewrite(self, db):
        ...
        ProjectStage.objects.create(
            ...
            stage_type="rewrite",
            status="completed",
            output_data={"raw_text": "改写后的文案内容", "human_text": "改写后的文案"},
        )

        # 创建storyboard阶段
        ProjectStage.objects.create(project=project, stage_type="storyboard", status="pending")
    ...
    def project_without_rewrite(self, db):
        ...
        project = Project.objects.create(
            name="测试项目2", description="测试描述2", original_topic="原始主题", user=user
        )
    ...
    def test_get_input_data_from_completed_rewrite(self, processor, project_with_rewrite):
        ...
        stage = ProjectStage.objects.get(project=project_with_rewrite, stage_type="storyboard")
    ...
    def test_get_input_data_without_rewrite_stage(self, processor, project_without_rewrite):
        ...
        stage = ProjectStage.objects.create(
            project=project_without_rewrite, stage_type="storyboard", status="pending"
        )
...
class TestStoryboardProcessorSaveResult:
    ...
    def project_with_stages(self, db):
        ...
        # 创建rewrite阶段
        ProjectStage.objects.create(project=project, stage_type="rewrite", status="completed")
    ...
    def test_save_result_updates_three_stages(self, mock_parse, processor, project_with_stages):
        """测试保存结果时更新3个后续阶段"""
        # Mock JSON解析
        ...
        storyboard_stage = ProjectStage.objects.get(
            project=project_with_stages, stage_type="storyboard"
        )
        ...
        for stage_type in ["image_generation", "camera_movement", "video_generation"]:
            stage = ProjectStage.objects.get(project=project_with_stages, stage_type=stage_type)
...
class TestStoryboardProcessorProcessStream:
    ...
    def project_with_templates_and_rewrite(self, db):
        ...
        template_set = PromptTemplateSet.objects.create(
            name="Test Set", is_default=True, created_by=user
        )
        ...
        ProjectStage.objects.create(
            ...
            stage_type="rewrite",
            status="completed",
            output_data={"raw_text": "改写后的文案", "human_text": "改写后的文案"},
        )
    ...
    def test_process_stream_yields_tokens(
        self, mock_create_client, processor, project_with_templates_and_rewrite
    ):
        """测试流式处理yield token消息"""
        # Mock AI客户端
        ...
        mock_client.generate_stream.return_value = [
            {
                ...
                "full_text": '{"scenes": [{"scene_number": 1,',
            },
            {
                "type": "token",
                "content": '"narration": "测试"}]}',
                "full_text": '{"scenes": [{"scene_number": 1, "narration": "测试"}]}',
            },
        ]
        ...
        chunks = list(
            processor.process_stream(
                project_id=str(project_with_templates_and_rewrite.id), input_data={}
            )
        )
    ...
    def test_process_stream_updates_stage_to_completed(
        ...
        stage = ProjectStage.objects.get(
            project=project_with_templates_and_rewrite, stage_type="storyboard"
        )
    ...
    def test_process_stream_handles_json_parsing_error(
        ...
        # Mock AI客户端 - 返回无效JSON
        mock_client = MagicMock()
        mock_client.generate_stream.return_value = [{"type": "done", "text": "invalid json"}]
        mock_create_client.return_value = mock_client
...
class TestStoryboardProcessorValidate:
    ...
    def project_with_completed_rewrite(self, db):
        ...
        project = Project.objects.create(name="测试项目", original_topic="原始主题", user=user)
