...
class TestImage2VideoProcessorValidate:
    ...
    def project_with_completed_stages(self, db):
        ...
        project = Project.objects.create(
            name="测试项目", description="测试描述", original_topic="原始主题", user=user
        )
    ...
    def test_validate_with_completed_prerequisites(
        self, processor, project_with_completed_stages, model_provider
    ):
        """测试前置阶段都完成时验证成功"""
        # 配置模型
        from apps.projects.models import ProjectModelConfig

        config = ProjectModelConfig.objects.create(project=project_with_completed_stages)
    ...
    def test_validate_without_image_generation_fails(
        ...
        ProjectStage.objects.filter(
            project=project_with_completed_stages, stage_type="image_generation"
        ).delete()
    ...
    def test_validate_without_camera_movement_fails(
        ...
        # 删除camera_movement阶段
        ProjectStage.objects.filter(
            project=project_with_completed_stages, stage_type="camera_movement"
        ).delete()
    ...
    def test_validate_without_image_urls_fails(
        ...
        image_stage.output_data = {
            "human_text": {
                "scenes": [{"scene_number": 1, "narration": "开场", "visual_prompt": "提示"}]
            }
        }
        image_stage.save()
    ...
    def test_validate_without_provider_fails(self, processor, project_with_completed_stages):
        """测试无模型配置时验证失败"""
        # 不配置模型
        context = PipelineContext(project_id=str(project_with_completed_stages.id))
        result = processor.validate(context)
...
class TestImage2VideoGenerateSingleVideo:
    ...
    def project_with_data(self, db):
        ...
        project = Project.objects.create(name="测试项目", original_topic="原始", user=user)
    ...
    def test_generate_single_video_success(
        self, mock_create_client, processor, project_with_data, model_provider, storyboard_dict
    ):
        """测试成功生成单个视频"""
        # Mock AI客户端
        mock_client = MagicMock()
        mock_response = {"data": [{"url": "http://example.com/video1.mp4"}]}
        mock_client._generate_video.return_value = mock_response
        mock_create_client.return_value = mock_client

        # Mock _build_prompt和image_to_base64
        with (
            patch.object(processor, "_build_prompt", return_value="mock prompt"),
            patch.object(processor, "image_to_base64", return_value="base64string"),
        ):
            # 生成视频
            chunks = list(
                processor._generate_single_video_stream(
    ...
    def test_generate_single_video_no_image_urls(
        self, processor, project_with_data, model_provider
    ):
        """测试没有图片URL时返回error"""
        storyboard_dict = {
            "scene_number": 1,
...
class TestImage2VideoSaveResult:
    ...
    def test_save_result_updates_video_urls(self, processor, project_with_stages):
        """测试保存结果时更新video_urls"""
        video_stage = ProjectStage.objects.get(
            project=project_with_stages, stage_type="video_generation"
        )
...
class TestImage2VideoProcessStream:
    ...
    @patch(
        "apps.content.processors.image2video_stage.Image2VideoStageProcessor._generate_single_video_stream"
    )
    def test_process_stream_yields_progress(self, mock_generate, processor, project_with_data):
        """测试流式处理yield进度消息"""
        # Mock视频生成
        ...
        chunks = list(processor.process_stream(project_id=str(project_with_data.id)))
    ...
    def test_process_stream_yields_video_generated(
        self, mock_generate, processor, project_with_data
    ):
        ...
        mock_generate.side_effect = [
            iter(
                [
                    {
                        "type": "video_generated",
                        "video_urls": [{"url": "http://example.com/video1.mp4"}],
                    }
                ]
            ),
            iter([{"type": "error", "error": "API Error"}]),
        ]
...
class TestImage2VideoBuildPrompt:
    ...
    def project_with_template(self, db):
        ...
        template_set = PromptTemplateSet.objects.create(
            name="Video Template Set", is_default=True, created_by=user
        )
        ...
        project = Project.objects.create(
            name="测试项目", original_topic="原始", user=user, prompt_template_set=template_set
        )
...
class TestImage2VideoGetProvider:
    ...
    def test_get_provider_returns_default_when_no_config(self, processor, db):
        ...
        project = Project.objects.create(name="测试项目2", original_topic="原始", user=user)
