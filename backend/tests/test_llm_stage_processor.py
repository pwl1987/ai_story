...
class TestLLMStageProcessorProcessStream:
    ...
    def mock_project(self, db):
        ...
        project = Project.objects.create(
            name="测试项目", description="测试描述", original_topic="原始主题内容", user=user
        )
        return project
    ...
    def project_with_stages(self, mock_project):
        ...
        # 创建提示词集
        template_set = PromptTemplateSet.objects.create(
            name="Test Set for Stream", is_default=True, created_by=mock_project.user
        )
        ...
        for stage_type in [
            ...
            stage = ProjectStage.objects.create(
                project=mock_project, stage_type=stage_type, status="pending"
            )
            stages.append(stage)
    ...
    def test_process_stream_yields_stage_update_first(
        self, mock_template_query, mock_create_client, processor, project_with_stages
    ):
        ...
        # 执行流式处理
        chunks = list(
            processor.process_stream(project_id=str(project_with_stages.id), input_data={})
        )
    ...
    def test_process_stream_updates_stage_to_completed(
        ...
        # Mock AI客户端
        mock_client = MagicMock()
        mock_client.generate_stream.return_value = [{"type": "done", "text": "Final text"}]
        mock_create_client.return_value = mock_client
        ...
        # 获取初始阶段
        stage = ProjectStage.objects.get(project=project_with_stages, stage_type="rewrite")
        assert stage.status == "pending"

        # 执行流式处理
        list(processor.process_stream(project_id=str(project_with_stages.id), input_data={}))
...
class TestLLMStageProcessorOnFailure:
    ...
    def project_with_stage(self, db):
        ...
        ProjectStage.objects.create(project=project, stage_type="rewrite", status="processing")
        return project
    ...
    def test_on_failure_updates_stage_status(self, processor, project_with_stage):
        ...
        # 验证状态
        stage = ProjectStage.objects.get(project=project_with_stage, stage_type="rewrite")
        assert stage.status == "failed"
...
class TestLLMStageProcessorHelperMethods:
    ...
    def test_get_ai_client_from_template(
        self, mock_create_client, processor, project_with_templates
    ):
        """测试从提示词模板获取AI客户端"""
        mock_client = MagicMock()
        mock_create_client.return_value = mock_client
    ...
    def test_save_result_for_rewrite(self, mock_parse, processor, project_with_templates):
        """测试保存rewrite阶段结果"""
        from apps.projects.models import ProjectStage

        # 创建阶段
        stage = ProjectStage.objects.create(project=project_with_templates, stage_type="rewrite")
    ...
    @patch("apps.content.processors.llm_stage.parse_storyboard_json")
    def test_save_result_for_storyboard(self, mock_parse, project_with_templates):
        """测试保存storyboard阶段结果"""
        from apps.projects.models import ProjectStage
        ...
        # 创建阶段
        stage = ProjectStage.objects.create(project=project_with_templates, stage_type="storyboard")
