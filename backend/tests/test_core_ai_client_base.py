...
class TestAIResponse:
    ...
    def test_create_error_response(self):
        """测试创建错误响应"""
        response = AIResponse(success=False, error="API call failed")
    ...
    def test_response_post_init_populates_defaults(self):
        ...
        response = AIResponse(success=True, text="Test")
...
class TestBaseAIClient:
    ...
    def concrete_client(self):
        ...
        return ConcreteAIClient(
            api_url="https://api.example.com", api_key="test_key", model_name="test_model"
        )
    ...
    async def test_health_check_failure(self):
        ...
        client = FailingClient(
            api_url="https://api.example.com", api_key="invalid_key", model_name="test_model"
        )
...
class TestLLMClient:
    ...
    def concrete_llm_client(self):
        ...
        class ConcreteLLMClient(LLMClient):
            async def _generate_text(self, prompt, max_tokens, temperature, **kwargs):
                return AIResponse(
                    success=True, text=f"LLM generated: {prompt}", metadata={"tokens": max_tokens}
                )
        ...
        return ConcreteLLMClient(
            api_url="https://api.openai.com", api_key="test_key", model_name="gpt-4"
        )
    ...
    async def test_llm_generate_with_custom_params(self, concrete_llm_client):
        ...
        response = await concrete_llm_client.generate(
            prompt="Test prompt", max_tokens=500, temperature=0.5
        )
...
class TestText2ImageClient:
    ...
    def concrete_image_client(self):
        ...
        class ConcreteImageClient(Text2ImageClient):
            async def _generate_image(
                self, prompt, negative_prompt="", width=1024, height=1024, steps=20, **kwargs
            ):
                return AIResponse(
                    success=True,
                    data={
        ...
        return ConcreteImageClient(
            api_url="https://api.stability.ai", api_key="test_key", model_name="stable-diffusion"
        )
    ...
    async def test_image_generate_custom_params(self, concrete_image_client):
        ...
        response = await concrete_image_client.generate(
            prompt="A sunset", width=512, height=512, steps=30
        )
...
class TestImage2VideoClient:
    ...
    def concrete_video_client(self):
        ...
        class ConcreteVideoClient(Image2VideoClient):
            async def _generate_video(self, image_url, camera_movement, duration, fps, **kwargs):
                return AIResponse(
                    success=True,
                    data={
        ...
        return ConcreteVideoClient(
            api_url="https://api.runway.ml", api_key="test_key", model_name="gen2"
        )
