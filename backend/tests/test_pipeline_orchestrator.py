...
class MockSuccessProcessor(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        ...
        return StageResult(success=True, data={"stage": self.stage_name, "result": "success"})
...
class MockFailureProcessor(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        """处理返回失败"""
        return StageResult(success=False, error=f"{self.stage_name} failed")
...
class MockValidationProcessor(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        """不应该被调用"""
        return StageResult(success=True, data={"test": "data"})
