...
pytestmark = [
    ...
    pytest.mark.skipif(not pytest.importorskip("redis"), reason="需要redis包"),
]
...
class TestRealtimeMessaging:
    ...
    def test_publish_stage_update_message(self, redis_client):
        ...
        result = publisher.publish_stage_update(
            status="processing", progress=50, message="正在生成分镜..."
        )
    ...
    def test_publish_error_message(self, redis_client):
        ...
        # 发布错误消息
        result = publisher.publish_error(error="生成失败", retry_count=1)
    ...
    def test_publish_done_message(self, redis_client):
        ...
        # 发布完成消息
        result = publisher.publish_done(full_text="运镜生成完成", metadata={"frames": 120})
    ...
    def test_context_manager(self, redis_client):
        ...
        with RedisStreamPublisher(project_id, stage_name) as publisher:
            result = publisher.publish_stage_update(
                status="completed", progress=100, message="完成"
            )
            assert result is True
        # 连接应该自动关闭
    ...
    def test_different_stages(self, redis_client):
        ...
        for stage in stages:
            ...
            result = publisher.publish_stage_update(
                status="processing", progress=0, message=f"{stage} 开始"
            )
            assert result is True
            publisher.close()
    ...
    def test_publish_progress_message(self, redis_client):
        ...
        # 发布进度更新
        result = publisher.publish_progress(current=75, total=100, item_name="视频生成")
    ...
    def test_message_data_structure(self, redis_client):
        ...
        result = publisher.publish_done(full_text="完成", metadata={"step": 1, "total_steps": 3})
