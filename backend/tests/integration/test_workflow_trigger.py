...
class TestWorkflowTrigger:
    ...
    def test_execute_llm_stage_triggers_celery_task(
        self, mock_delay, api_client, test_user, project_with_stages
    ):
        """测试执行LLM阶段触发Celery任务"""
        ...
        request_data = {
            "stage_name": "rewrite",
            "input_data": {"raw_text": "AI视频生成测试", "human_text": ""},
        }

        # 发送POST请求执行阶段
        response = api_client.post(
            f"/api/v1/projects/{project_with_stages.id}/execute_stage/", request_data, format="json"
        )
    ...
    @patch("apps.projects.tasks.execute_llm_stage.delay")
    def test_execute_stage_updates_project_status(
        self, mock_delay, api_client, test_user, project_with_stages
    ):
        ...
        # 执行阶段
        request_data = {"stage_name": "storyboard", "input_data": {"raw_text": "测试"}}
    ...
    def test_execute_image_generation_stage(
        self, mock_delay, api_client, test_user, project_with_stages
    ):
        """测试执行文生图阶段"""
        api_client.force_authenticate(user=test_user)
        ...
        request_data = {
            ...
            "input_data": {"storyboard_ids": ["sb-1", "sb-2"]},
        }
    ...
    def test_execute_video_generation_stage(
        self, mock_delay, api_client, test_user, project_with_stages
    ):
        """测试执行图生视频阶段"""
        api_client.force_authenticate(user=test_user)
    ...
    def test_execute_invalid_stage_type(self, api_client, test_user, project_with_stages):
        ...
        request_data = {"stage_name": "invalid_stage", "input_data": {}}
    ...
    def test_execute_stage_unauthenticated(self, api_client, project_with_stages):
        """测试未认证用户无法执行阶段"""
        request_data = {"stage_name": "rewrite", "input_data": {}}
    ...
    @patch("apps.projects.tasks.execute_llm_stage.delay")
    def test_execute_stage_channel_format(
        self, mock_delay, api_client, test_user, project_with_stages
    ):
        """测试Redis频道名称格式正确"""
        api_client.force_authenticate(user=test_user)
        ...
        request_data = {"stage_name": "camera_movement", "input_data": {}}
    ...
    def test_execute_nonexistent_project(self, mock_delay, api_client, test_user):
        ...
        response = api_client.post(
            f"/api/v1/projects/{fake_project_id}/execute_stage/", request_data, format="json"
        )
    ...
    @patch("apps.projects.tasks.execute_llm_stage.delay")
    def test_execute_stage_project_already_processing(
        self, mock_delay, api_client, test_user, project_with_stages
    ):
        """测试项目已在处理中时不重复更新状态"""
        api_client.force_authenticate(user=test_user)
