...
class TestStoryboardE2E:
    ...
    def configured_user(self, db):
        ...
        user = User.objects.create_user(
            username='storyboard_test_user',
            email='storyboard@test.com'
        )
        return user
    ...
    def test_storyboard_updates_subsequent_stages(
        ...
        ProjectStage.objects.filter(
            project=Project.objects.get(id=project_id),
            stage_type__in=["storyboard", "image_generation", "camera_movement", "video_generation"]
        ).update(
