...
class TestModelProviderViewSet:
    ...
    def test_list_providers(self):
        """测试获取模型提供商列表"""
        ModelProviderFactory(name="OpenAI", provider_type="llm", is_active=True)
        ModelProviderFactory(name="Stable Diffusion", provider_type="text2image", is_active=True)
    ...
    def test_list_providers_filtered_by_status(self):
        """测试按激活状态过滤"""
        ModelProviderFactory(name="Active Provider", provider_type="llm", is_active=True)
        ModelProviderFactory(name="Inactive Provider", provider_type="llm", is_active=False)
    ...
    def test_search_providers(self):
        """测试搜索模型提供商"""
        ModelProviderFactory(name="GPT-4 Provider", provider_type="llm", is_active=True)
        ModelProviderFactory(name="Claude Provider", provider_type="llm", is_active=True)
    ...
    def test_retrieve_provider(self):
        """测试获取模型提供商详情"""
        provider = ModelProviderFactory(name="Test Provider", provider_type="llm", is_active=True)
    ...
    def test_update_provider(self):
        """测试更新模型提供商"""
        provider = ModelProviderFactory(name="Original Name", provider_type="llm", is_active=True)
    ...
    def test_delete_provider(self):
        """测试删除模型提供商"""
        provider = ModelProviderFactory(name="To Delete", provider_type="llm", is_active=True)
    ...
    def test_toggle_status_inactive_to_active(self):
        """测试切换状态 - 从停用到激活"""
        provider = ModelProviderFactory(name="Test Provider", provider_type="llm", is_active=False)
    ...
    def test_get_provider_statistics(self):
        ...
        provider = ModelProviderFactory(name="Test Provider", provider_type="llm", is_active=True)
        # 创建一些使用日志
        ModelUsageLogFactory(model_provider=provider, status="success", tokens_used=1000)
        ModelUsageLogFactory(model_provider=provider, status="success", tokens_used=2000)
    ...
    def test_test_connection(self):
        ...
        response = self.client.post(
            f"/api/v1/models/providers/{provider.id}/test_connection/", data, format="json"
        )
    ...
    def test_get_usage_logs(self):
        """测试获取使用日志"""
        provider = ModelProviderFactory(name="Test Provider", provider_type="llm", is_active=True)
        ModelUsageLogFactory(model_provider=provider, status="success")
    ...
    def test_get_active_providers_by_type(self):
        """测试按类型获取激活的模型提供商"""
        ModelProviderFactory(name="LLM Provider", provider_type="llm", is_active=True)
        ModelProviderFactory(name="Image Provider", provider_type="text2image", is_active=True)
    ...
    def test_get_providers_by_type(self):
        """测试按类型分组获取模型提供商"""
        ModelProviderFactory(name="GPT-4", provider_type="llm", is_active=True)
        ModelProviderFactory(name="Stable Diffusion", provider_type="text2image", is_active=True)
    ...
    def test_get_simple_list(self):
        """测试获取简化的模型列表"""
        ModelProviderFactory(name="Provider 1", provider_type="llm", is_active=True)
        ModelProviderFactory(name="Provider 2", provider_type="llm", is_active=True)
    ...
    def test_get_executor_choices(self):
        ...
        assert response.status_code == status.HTTP_200_OK
        # 应该包含所有类型的执行器
        assert "llm" in response.data or any(
            key in response.data for key in ["llm", "text2image", "image2video"]
        )
...
class TestModelUsageLogViewSet:
    ...
    def setup_method(self):
        ...
        self.provider = ModelProviderFactory(
            name="Test Provider", provider_type="llm", is_active=True
        )

    def test_list_usage_logs(self):
        """测试获取使用日志列表"""
        ModelUsageLogFactory(model_provider=self.provider, status="success")
        ModelUsageLogFactory(model_provider=self.provider, status="failed")
    ...
    def test_retrieve_usage_log(self):
        """测试获取使用日志详情"""
        log = ModelUsageLogFactory(model_provider=self.provider, status="success")
    ...
    def test_filter_logs_by_project(self):
        ...
        ModelUsageLogFactory(
            model_provider=self.provider, status="success", project_id=test_project_id
        )
    ...
    def test_get_logs_by_project(self):
        ...
        ModelUsageLogFactory(model_provider=self.provider, status="success", project_id=project_id)
    ...
    def test_get_failed_logs(self):
        ...
        ModelUsageLogFactory(model_provider=self.provider, status="success")
        ModelUsageLogFactory(
            model_provider=self.provider, status="failed", error_message="API error"
        )
...
class TestModelValidation:
    ...
    def test_update_provider_priority(self):
        """测试更新提供商优先级"""
        provider = ModelProviderFactory(name="Test Provider", provider_type="llm", priority=10)
