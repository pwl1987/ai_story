...
class TestProgressHistoryRecorder:
    ...
    def setup(self):
        ...
        self.user = User.objects.create_user(
            username="testuser_recorder", email="test_recorder@example.com"
        )

        # 创建测试项目
        self.project = Project.objects.create(
            name="测试项目", original_topic="测试主题", user=self.user
        )
    ...
    def test_record_done(self):
        ...
        history = ProgressHistoryRecorder.record_done(
            project_id=str(self.project.id), stage="storyboard", result=result, metadata=metadata
        )
    ...
    def test_multiple_records_same_stage(self):
        ...
        histories = ProjectProgressHistory.objects.filter(project=self.project, stage="rewrite")
...
class TestProgressHistoryQuery:
    ...
    def setup(self):
        ...
        # 创建测试用户和项目
        self.user = User.objects.create_user(
            username="testuser_query", email="test_query@example.com"
        )
        self.project = Project.objects.create(
            name="测试项目", original_topic="测试主题", user=self.user
        )
    ...
    def _create_test_data(self):
        ...
        ProgressHistoryRecorder.record_token(
            project_id=str(self.project.id), stage="rewrite", content="token1", full_text="token1"
        )

    def test_get_project_history_all(self):
        """测试查询项目所有历史"""
        history = ProgressHistoryQuery.get_project_history(
            project_id=str(self.project.id), limit=100
        )
    ...
    def test_get_project_history_filter_by_stage(self):
        """测试按阶段过滤"""
        history = ProgressHistoryQuery.get_project_history(
            project_id=str(self.project.id), stage="rewrite", limit=100
        )
    ...
    def test_get_project_history_with_limit(self):
        """测试限制返回数量"""
        history = ProgressHistoryQuery.get_project_history(project_id=str(self.project.id), limit=5)
    ...
    def test_get_project_history_with_offset(self):
        ...
        page1 = ProgressHistoryQuery.get_project_history(
            project_id=str(self.project.id), limit=5, offset=0
        )

        # 获取后5条
        page2 = ProgressHistoryQuery.get_project_history(
            project_id=str(self.project.id), limit=5, offset=5
        )
    ...
    def test_get_stage_history(self):
        ...
        history = ProgressHistoryQuery.get_stage_history(
            project_id=str(self.project.id), stage="storyboard", limit=10
        )
    ...
    def test_get_latest_progress_single_stage(self):
        ...
        latest = ProgressHistoryQuery.get_latest_progress(
            project_id=str(self.project.id), stage="rewrite"
        )
    ...
    def test_get_latest_progress_all_stages(self):
        """测试获取所有阶段的最新进度"""
        latest = ProgressHistoryQuery.get_latest_progress(project_id=str(self.project.id))
    ...
    def test_get_history_stats(self):
        """测试获取历史统计信息"""
        stats = ProgressHistoryQuery.get_history_stats(project_id=str(self.project.id))
    ...
    def test_cleanup_old_history(self):
        ...
        deleted_count = ProgressHistoryQuery.cleanup_old_history(
            project_id=str(self.project.id), days_old=30
        )
    ...
    def test_query_nonexistent_project(self):
        ...
        with pytest.raises(Project.DoesNotExist):
            ProgressHistoryQuery.get_project_history(
                project_id="00000000-0000-0000-0000-000000000000", limit=10
            )
...
class TestProgressHistoryIntegration:
    ...
    def setup(self):
        ...
        self.project = Project.objects.create(
            name="集成测试项目", original_topic="集成测试主题", user=self.user
        )
