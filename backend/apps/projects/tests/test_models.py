...
class TestProjectModel:
    ...
    def test_create_project_minimal(self):
        """测试创建最小项目"""
        user = UserFactory()
        project = Project.objects.create(name="测试项目", original_topic="人工智能", user=user)
...
class TestProjectStageModel:
    ...
    def test_create_stage_minimal(self):
        """测试创建最小阶段"""
        project = ProjectFactory()
        stage = ProjectStage.objects.create(project=project, stage_type="rewrite")
    ...
    def test_stage_type_choices(self):
        ...
        for stage_type in stage_types:
            stage = ProjectStageFactory(project=project, stage_type=stage_type)
    ...
    def test_stage_json_fields(self):
        ...
        stage = ProjectStageFactory(project=project, input_data=input_data, output_data=output_data)
    ...
    def test_stage_retry_mechanism(self):
        """测试重试机制"""
        project = ProjectFactory()
        stage = ProjectStageFactory(project=project, retry_count=0, max_retries=3)
...
class TestProjectModelConfigModel:
    ...
    def test_config_load_balance_strategies(self):
        ...
        for strategy in strategies:
            project = ProjectFactory()
            config = ProjectModelConfigFactory(project=project, load_balance_strategy=strategy)
