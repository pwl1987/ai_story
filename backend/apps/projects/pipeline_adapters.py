...
class RewriteStageAdapter(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        ...
        stage, _created = await sync_to_async_wrapper(ProjectStage.objects.get_or_create)(
            project=project,
            stage_type="rewrite",
            defaults={"status": "pending", "input_data": {"raw_text": project.original_topic}},
        )
        ...
        try:
            ...
            def run_sync_processor():
                ...
                for chunk in self.processor.process_stream(
                    project_id=project.id, input_data={"raw_text": project.original_topic}
                ):
                    ...
                    if chunk_type == "token":
                        ...
                        return StageResult(success=True, data={"rewritten_text": full_text})
    ...
    async def on_failure(self, context: PipelineContext, error: Exception):
        ...
        try:
            stage = await sync_to_async_wrapper(ProjectStage.objects.get)(
                project_id=context.project_id, stage_type="rewrite"
            )
            stage.status = "failed"
...
请生成场景大纲："""
            ...
            async def call_llm():
                ...
                logger.info(
                    f"LLM响应: success={response.success}, text_length={len(response.text) if response.text else 0}"
                )
...
class ImageGenerationStageAdapter(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        ...
        # 获取或创建阶段
        stage, _created = await sync_to_async_wrapper(ProjectStage.objects.get_or_create)(
            project=project, stage_type="image_generation", defaults={"status": "pending"}
        )
        ...
        try:
            ...
            logger.info(
                f"项目 {project.id}: 图片生成完成，成功 {success_count}/{len(scenes)}，失败 {failed_count}"
            )
    ...
    async def _generate_scene_image(
        self, scene_number: int, scene: Dict[str, Any], project: Project
    ) -> Dict[str, Any]:
        """
        为单个场景生成图片（异步版本）
    ...
    async def on_failure(self, context: PipelineContext, error: Exception):
        ...
        try:
            stage = await sync_to_async_wrapper(ProjectStage.objects.get)(
                project_id=context.project_id, stage_type="image_generation"
            )
            stage.status = "failed"
            stage.error_message = str(error)
            await sync_to_async_wrapper(stage.save)()
...
class CameraMovementStageAdapter(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        ...
        # 获取或创建阶段
        stage, _created = await sync_to_async_wrapper(ProjectStage.objects.get_or_create)(
            project=project, stage_type="camera_movement", defaults={"status": "pending"}
        )
        ...
        try:
            ...
            logger.info(
                f"项目 {project.id}: 运镜生成完成，成功 {success_count}/{len(scenes)}，失败 {failed_count}"
            )
    ...
    async def _generate_camera_movement(
        ...
        try:
            ...
            scene_description = (
                ...
                or scene.get("narration", "")  # Mock格式
                or scene.get("visual_prompt", "")  # 备用
            )
...
"""
            ...
            response = await client.generate(prompt=prompt, max_tokens=500, temperature=0.7)
            ...
            if response.success:
                ...
                try:
                    ...
                    return {
                        ...
                        "scene_number": scene_number,
                        "movement_type": "static",
                        "movement_params": {"start_scale": 1.0, "end_scale": 1.0, "duration": 3.0},
                    }
            else:
                error_msg = response.error or "未知错误"
                logger.error(f"场景 {scene_number} AI调用失败: {error_msg}")
    ...
    async def on_failure(self, context: PipelineContext, error: Exception):
        ...
        try:
            stage = await sync_to_async_wrapper(ProjectStage.objects.get)(
                project_id=context.project_id, stage_type="camera_movement"
            )
            stage.status = "failed"
            stage.error_message = str(error)
            await sync_to_async_wrapper(stage.save)()
...
class VideoGenerationStageAdapter(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        ...
        # 获取或创建阶段
        stage, _created = await sync_to_async_wrapper(ProjectStage.objects.get_or_create)(
            project=project, stage_type="video_generation", defaults={"status": "pending"}
        )
        ...
        try:
            ...
            if result.success:
                ...
                return StageResult(success=True, data={"videos": result.data.get("videos", [])})
            else:
    ...
    async def on_failure(self, context: PipelineContext, error: Exception):
        ...
        try:
            stage = await sync_to_async_wrapper(ProjectStage.objects.get)(
                project_id=context.project_id, stage_type="video_generation"
            )
            stage.status = "failed"
            stage.error_message = str(error)
            await sync_to_async_wrapper(stage.save)()
