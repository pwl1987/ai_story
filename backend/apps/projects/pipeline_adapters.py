...
class StoryboardStageAdapter(StageProcessor):
    ...
    async def process(self, context: PipelineContext) -> StageResult:
        ...
        try:
            ...
            logger.info(f"项目 {project.id}: 分镜生成完成，成功 {success_count}/{scene_count}，失败 {failed_count}")
    ...
    async def _generate_scene_outline(self, project: Project, rewritten_text: str) -> Dict[str, Any]:
        """
        阶段1: 快速生成场景大纲
...
请生成场景大纲："""
    ...
    async def _generate_scene_detail(self, scene_number: int, scene_outline: Dict[str, Any], project: Project) -> Dict[str, Any]:
        """
