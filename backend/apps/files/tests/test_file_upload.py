...
class FileUploadAPITestCase(APITestCase):
    ...
    def test_upload_image_success(self):
        ...
        response = self.client.post(
            "/api/v1/files/upload/", {"file": uploaded_file}, format="multipart"
        )
    ...
    def test_upload_document_success(self):
        ...
        with open(tmp_path, "rb") as f:
            uploaded_file = SimpleUploadedFile(
                "test_document.txt", f.read(), content_type="text/plain"
            )
    ...
    def test_upload_file_too_large(self):
        ...
        # 发送上传请求
        response = self.client.post(
            "/api/v1/files/upload/", {"file": large_file}, format="multipart"
        )
    ...
    def test_upload_invalid_file_type(self):
        ...
        invalid_file = SimpleUploadedFile(
            "test.exe", b"invalid executable", content_type="application/x-msdownload"
        )

        # 发送上传请求
        response = self.client.post(
            "/api/v1/files/upload/", {"file": invalid_file}, format="multipart"
        )
    ...
    def test_list_files(self):
        ...
        with open(tmp_path, "rb") as f:
            uploaded_file = SimpleUploadedFile("test_list.png", f.read(), content_type="image/png")

        self.client.post("/api/v1/files/upload/", {"file": uploaded_file}, format="multipart")
    ...
    def test_get_file_detail(self):
        ...
        with open(tmp_path, "rb") as f:
            uploaded_file = SimpleUploadedFile(
                "test_detail.jpg", f.read(), content_type="image/jpeg"
            )
    ...
    def test_delete_file(self):
        ...
        with open(tmp_path, "rb") as f:
            uploaded_file = SimpleUploadedFile(
                "test_delete.jpg", f.read(), content_type="image/jpeg"
            )
    ...
    def test_file_deduplication(self):
        ...
        uploaded_file1 = SimpleUploadedFile(
            "duplicate.jpg", file_content, content_type="image/jpeg"
        )
        response1 = self.client.post(
            "/api/v1/files/upload/", {"file": uploaded_file1}, format="multipart"
        )
        ...
        response2 = self.client.post(
            "/api/v1/files/upload/", {"file": uploaded_file2}, format="multipart"
        )

        # 验证第二次上传返回已存在的文件
        self.assertEqual(response2.status_code, status.HTTP_200_OK)
        self.assertEqual(response2.data["message"], "文件已存在")
        self.assertEqual(response1.data["file"]["id"], response2.data["file"]["id"])
