...
class UploadedFile(models.Model):
    ...
    file_type = models.CharField(
        "文件类型", max_length=20, choices=FILE_TYPE_CHOICES, default="other"
    )
    ...
    def clean(self):
        ...
        if self.file_type != expected_type:
            raise ValidationError({"file_type": f"文件类型不匹配，应该是 {expected_type}"})
...
class FileQuota(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="file_quota", verbose_name="用户"
    )
