...
class TestPromptEvaluationService:
    ...
    async def test_evaluate_prompt_success(self):
        ...
        template = await sync_to_async(PromptTemplateFactory)(
            template_content="测试提示词: {{topic}}", variables={"topic": "string"}
        )
    ...
    async def test_evaluate_prompt_ai_failure(self):
        ...
        # When & Then
        with (
            patch.object(service, "_get_ai_client", return_value=mock_client),
    ...
    async def test_compare_promps_equal_scores(self):
        ...
        mock_response = Mock()
        mock_response.success = True
        mock_response.data = {"score": 8.0, "clarity": 8.0, "specificity": 8.0, "creativity": 8.0}
    ...
    async def test_suggest_improvements(self):
        ...
        # Given
        service = PromptEvaluationService()
        template = await sync_to_async(PromptTemplateFactory)(template_content="简单提示词")
    ...
    async def test_compare_promps_all_dimensions(self):
        ...
        eval1 = {"score": 7.0, "clarity": 6.0, "specificity": 8.0, "creativity": 7.0}

        eval2 = {"score": 8.0, "clarity": 9.0, "specificity": 7.0, "creativity": 8.0}
