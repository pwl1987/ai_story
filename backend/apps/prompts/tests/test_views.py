...
class TestPromptTemplateSetViewSet:
    ...
    def test_list_prompt_sets(self):
        ...
        PromptTemplateSet.objects.create(
            name="测试集1", is_active=True, is_default=True, created_by=self.admin_user
        )
        PromptTemplateSet.objects.create(name="我的集", is_active=True, created_by=self.user)
    ...
    def test_create_prompt_set(self):
        """测试创建提示词集"""
        data = {"name": "新提示词集", "description": "测试描述", "is_active": True}
    ...
    def test_retrieve_prompt_set(self):
        ...
        prompt_set = PromptTemplateSet.objects.create(
            name="测试集", is_active=True, created_by=self.user
        )
    ...
    def test_update_prompt_set(self):
        """测试更新提示词集"""
        prompt_set = PromptTemplateSet.objects.create(
            name="原始名称", is_active=True, created_by=self.user
        )
    ...
    def test_delete_prompt_set(self):
        """测试删除提示词集"""
        prompt_set = PromptTemplateSet.objects.create(
            name="待删除", is_active=True, created_by=self.user
        )
    ...
    def test_clone_prompt_set(self):
        ...
        original_set = PromptTemplateSet.objects.create(
            name="原始集", is_active=True, created_by=self.user
        )
        # 创建模板
        PromptTemplate.objects.create(
            template_set=original_set, stage_type="rewrite", template_content="原始内容"
        )
    ...
    def test_clone_missing_name(self):
        """测试克隆提示词集 - 缺少名称"""
        prompt_set = PromptTemplateSet.objects.create(name="测试集", created_by=self.user)
    ...
    def test_set_default_prompt_set(self):
        """测试设置为默认提示词集"""
        prompt_set = PromptTemplateSet.objects.create(name="默认集", created_by=self.admin_user)
    ...
    def test_get_default_prompt_set(self):
        ...
        default_set = PromptTemplateSet.objects.create(
            name="默认集", is_default=True, created_by=self.admin_user
        )
...
class TestPromptTemplateViewSet:
    ...
    def setup_method(self):
        ...
        self.template_set = PromptTemplateSet.objects.create(name="测试集", created_by=self.user)

    def test_list_templates(self):
        """测试获取模板列表"""
        PromptTemplate.objects.create(
            template_set=self.template_set, stage_type="rewrite", template_content="测试内容"
        )
    ...
    def test_retrieve_template(self):
        ...
        template = PromptTemplate.objects.create(
            template_set=self.template_set, stage_type="storyboard", template_content="测试内容"
        )
    ...
    def test_update_template(self):
        """测试更新模板"""
        template = PromptTemplate.objects.create(
            template_set=self.template_set, stage_type="rewrite", template_content="原始内容"
        )
    ...
    def test_delete_template(self):
        """测试删除模板"""
        template = PromptTemplate.objects.create(
            template_set=self.template_set, stage_type="rewrite", template_content="测试"
        )
    ...
    def test_create_version(self):
        ...
        data = {"template_content": "新版本内容", "variables": {"topic": "string"}}

        response = self.client.post(
            f"/api/v1/prompts/templates/{original_template.id}/create_version/", data, format="json"
        )
        ...
        if response.status_code == status.HTTP_201_CREATED:
            # 验证新版本创建成功
            new_template = (
                PromptTemplate.objects.filter(template_set=self.template_set, stage_type="rewrite")
                .order_by("-version")
                .first()
            )
    ...
    def test_validate_template(self):
        ...
        response = self.client.post(f"/api/v1/prompts/templates/{template.id}/validate/", data)
    ...
    def test_preview_template(self):
        ...
        data = {"variables": {"topic": "科幻故事", "style": "赛博朋克"}}

        response = self.client.post(
            f"/api/v1/prompts/templates/{template.id}/preview/", data, format="json"
        )
...
class TestGlobalVariableViewSet:
    ...
    def test_list_variables(self):
        ...
        # 创建用户级变量
        GlobalVariable.objects.create(
            key="user_var", value="用户值", scope="user", group="用户", created_by=self.user
        )
    ...
    def test_retrieve_variable(self):
        ...
        variable = GlobalVariable.objects.create(
            key="test_var", value="test_value", scope="user", created_by=self.user
        )
    ...
    def test_update_variable(self):
        """测试更新变量"""
        variable = GlobalVariable.objects.create(
            key="test_var", value="原始值", scope="user", created_by=self.user
        )
    ...
    def test_delete_user_variable(self):
        """测试删除用户级变量"""
        variable = GlobalVariable.objects.create(
            key="test_var", value="test", scope="user", created_by=self.user
        )
    ...
    def test_delete_system_variable_requires_admin(self):
        """测试删除系统级变量需要管理员权限"""
        variable = GlobalVariable.objects.create(
            key="system_var", value="system", scope="system", created_by=self.admin_user
        )
    ...
    def test_get_variable_groups(self):
        """测试获取变量分组"""
        GlobalVariable.objects.create(
            key="var1", value="val1", scope="user", group="分组1", created_by=self.user
        )
        GlobalVariable.objects.create(
            key="var2", value="val2", scope="user", group="分组2", created_by=self.user
        )
    ...
    def test_validate_key_valid(self):
        """测试验证变量键 - 有效"""
        data = {"key": "my_variable", "scope": "user"}
...
class TestPromptPermissions:
    ...
    def test_user_can_only_access_own_template_sets(self):
        ...
        # 创建其他用户的非默认集
        PromptTemplateSet.objects.create(
            name="其他用户的集", is_default=False, created_by=self.other_user
        )
        # 创建默认集
        default_set = PromptTemplateSet.objects.create(
            name="默认集", is_default=True, created_by=self.other_user
        )
