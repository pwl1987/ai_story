...
class TestLogQueryTool:
    ...
    def test_query_by_logger(self):
        """测试按logger过滤"""
        results = query_logs_from_file(self.log_file, filters={"logger": "apps.api"})
        assert len(results) == 2
        assert all(r["logger"] == "apps.api" for r in results)

    def test_query_by_level(self):
        """测试按日志级别过滤"""
        results = query_logs_from_file(self.log_file, filters={"level": "ERROR"})
    ...
    def test_query_slow_requests(self):
        ...
        results = query_logs_from_file(
            self.log_file, filters={"extra_fields.is_slow_request": True}
        )
        assert len(results) == 1
    ...
    def test_query_by_event(self):
        """测试按事件类型过滤"""
        results = query_logs_from_file(
            self.log_file, filters={"extra_fields.event": "task_failure"}
        )
        assert len(results) == 1
    ...
    def test_query_with_since_filter(self):
        ...
        results = query_logs_from_file(self.log_file, since="1h")
