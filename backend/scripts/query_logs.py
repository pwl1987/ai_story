                        ...
                        if log_time_str:
                            try:
                                ...
                                log_time = datetime.strptime(
                                    log_time_str.split(",")[0], "%Y-%m-%d %H:%M:%S"
                                )
...
def main():
    ...
    parser.add_argument(
        "--log-file", default="logs/app.log", help="日志文件路径 (默认: logs/app.log)"
    )
    ...
    parser.add_argument("--is-slow-request", type=str, help="按慢请求过滤 (true/false)")
    parser.add_argument("--task-name", help="按任务名过滤")
    parser.add_argument("--limit", type=int, default=50, help="返回结果数量限制 (默认: 50)")
    parser.add_argument("--since", help="时间范围 (如: 1h, 30m)")
    parser.add_argument(
        "--format", choices=["text", "json"], default="text", help="输出格式 (默认: text)"
    )
    parser.add_argument("--watch", action="store_true", help="监控模式，持续输出新日志")
    ...
    results = query_logs_from_file(
        args.log_file, filters=filters if filters else None, limit=args.limit, since=args.since
    )
